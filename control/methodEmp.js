...
const me = {
    ...
    deleteDep: async function (req, cb) {
        try {
            ...
            if (ret.length != 0) {
                let dep = await me.getDep({ body: { dep_id: dep_id } });
            ...
            (async function () {
                try {
                    ...
                    for (let i = 0; i < emp_id_arr.length; i++) {
                        ...
                        let user = await me.getEmp({ body: { emp_id: val } });
     ...
     * @returns {JsonString} {role:x, emp:x}
     */
    getRoleInfo: async function (req, cb) {
        try {
            ...
            if (rslt.length != 0) {
                delete eval(rslt[0]).password;
            }
            cb(JSON.stringify({ role: rst, emp: rslt }));
        } catch (err) {
            log.errLog(req)
            cb('error');
        }
    ...
    getAllResourcesChecked: async function (req, cb) {
        try {
            ...
            if (type == 'role') {
                ...
                cb(JSON.stringify({ all: rslt, check: rslt2 }))
            } else if (type == 'user') {
    ...
    outputUsers: async function (results, callback) {
        ...
        fs.writeFile(pathStr, buffer, 'binary', function (err) {
            if (err) {
                ...
                callback('true', `/${excelFile}/${timeStr}.xlsx`);
            }
        });
    }
}
