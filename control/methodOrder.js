...
var md = {
  ...
  getSortFlowState: promise.promisify(async function (req, cb) {
    try {
      ...
      cb(null, { flows: sortFlows, states: state_details_arr });
    } catch (err) {
      log.errLog(req);
      cb('error');
    }
  ...
  getFlows: promise.promisify(async function (req, cb) {
    try {
      ...
      cb(null, { flow_id: flow_id, order_id: order_id, flows: flows, arr2: arr2, count2: count2 });
    } catch (err) {
      log.errLog(req);
      cb('error');
    }
  ...
  getStatess: promise.promisify(async function (req, cb) {
    try {
      ...
      cb(JSON.stringify({ 'data': { orderState: arr1, states: arr2, flow_name: flow_name } }));
    } catch (err) {
      log.errLog(req);
      cb('error');
    }
   ...
   */
  writeExcel: async function (results, req, callback) {
    try {
...
let getPowerOrder = promise.promisify(async function (obj, cb) {
  try {
    ...
    let data;
    let jsBody = {
      appli_id: appli_id, order_type: order_type, type: type, channel_id: channel_id,
