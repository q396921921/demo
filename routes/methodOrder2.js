...
module.exports = {
  ...
  deleteOffPro: async function (body, cb) {
    try {
      let product_id = body.product_id;
      await otherOrder.deleteOffProduct([product_id]);
      cb('success');
    } catch (err) {
      debug("删除商品对应的内勤错误");
      cb('error');
  ...
  getOffByProduct_id: async function (body, cb) {
    try {
      let product_id = body.product_id;
      let office = queryOrder.getOffByProduct_id({ product_id: product_id }, [product_id]);
  ...
  getSortFlowState: async function (body, cb) {
    try {
      let flow_id = body.flow_id;
      let sortFlows = await queryOrder.getSortFlows([flow_id]);
      let arr2 = [];
      for (let i = 0; i < sortFlows.length; i++) {
        const flow_detail_id = sortFlows[i].flow_detail_id;
        let detailStates = await queryOrder.getdetailStates([flow_detail_id]);
        arr2.push(detailStates);
      }
      cb(JSON.stringify({ data: [sortFlows, arr2] }));
    } catch (err) {
      cb('error');
    }
  },
