...
// 跳转当前测试1页面，即全部与筛选      ✔
function jump(text) {
    window.location.href = '/product/product' + text + '?menu=' + text;
}
...
function getDetailFileTypes() {
    ...
    $.ajax({
        url: "/product/getDetail_file_type",
        type: "get",
        async: false,
        dataType: "text",
        success: function (result) {
...
function deleteDetailType(t) {
    ...
    if (flag) {
        ...
        $.ajax({
            url: "/product/deleteDetailFileType",
            type: "post",
            data: {
                detail_file_type_id: detail_file_type_id
            },
...
function setDetailChecked(file_type_id) {
    ...
    $.ajax({
        url: "/product/getDetailFile_types",
        type: "post",
        data: {
            file_type_id: file_type_id
        },
